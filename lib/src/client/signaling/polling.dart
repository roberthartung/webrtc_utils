...
class PollingSignalingChannel implements SignalingChannel {
  ...
  PollingSignalingChannel(this.pollingUrl) {
    HttpRequest.request(pollingUrl, method: 'POST', responseType: 'json', sendData: JSON.encode({'hello': null})).then((HttpRequest request) {
      // print('[$this] send request: ${request.responseText}');
      _poll();
    });
  ...
  void _poll() {
    ...
    HttpRequest.request(pollingUrl, method: 'GET', responseType: 'json').then((HttpRequest request) {
      print('[$this] Polling request: $request');
    });
    /*
    new Timer(new Duration(), () {
  ...
  void send(SignalingMessage message) {
    ...
    */
    ...
    HttpRequest.request(pollingUrl, method: 'POST', responseType: 'json', sendData: _converter.encode(message)).then((HttpRequest request) {
      print('[$this] send request: $request');
    });
  }
